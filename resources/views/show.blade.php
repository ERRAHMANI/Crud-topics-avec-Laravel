   ...
   <div class="card">
      <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center">
                    ...
                    <small><strong> Posté le  </strong>{{ $topic->created_at->format('d/m/Y à H:m')}}</small>
                    <span class="badge badge-secondary">{{ $topic->user->name }}</span>
            ...
            @forelse ($topic->comments as $comment)
                 <div class="card mb-2">
                     <div class="card-body d-flex justify-content-between">
                       <div> 
                            {{ $comment->content }}
                                <div class="d-flex justify-content-between align-items-center">
                                <small><strong> Posté le  </strong>{{ $comment->created_at->format('d/m/Y')}}</small>
                        ...
                        <div>
                               ...
                               <solution-button topic-id="{{ $topic->id }}" comment-id="{{ $comment->id }}">
                            </solution-button>
                 ...
                 <div class="card mb-2 ml-5">
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                               ...
                               <small><strong> Posté le  </strong>{{ $replyComment->created_at->format('d/m/Y') }}</small>
                               <span class="badge badge-secondary">{{ $replyComment->user->name }}</span>
                ...
                <form action="{{ route('comments.storeReply', $comment) }}" method="POST" class="mb-3 ml-5 d-none" id="replyComment-{{ $comment->id }}">
                  ...
                  <button type="submit" class="btn btn-primary">Réponde à ce commentaire</button>
                 </form>
                 @endauth
            @empty
